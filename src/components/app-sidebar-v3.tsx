    ...
    <div className="flex h-full w-full" style={{ display: 'contents', flexDirection: 'row' }}>
      ...
      {!isAccountInactive && activeItem && (() => {
        ...
        return (
          <Sidebar 
            collapsible="none" 
            className="w-72 border-l border-r flex-shrink-0 hidden md:flex transition-all duration-500 ease-[cubic-bezier(0.4,0,0.2,1)] animate-in slide-in-from-left h-screen sticky top-0" 
          >
            <div className="flex h-full flex-col">
              <SidebarHeader className="gap-2 border-b p-4 bg-gradient-to-r from-sidebar-accent/30 to-sidebar-accent/10">
                        ...
                        <SidebarMenuItem key={`${subItem.title}-${index}`}>
                          <SidebarMenuSubButton 
                            ...
                            className={`
                              group relative
                              ${isSubActive 
                                ? "bg-sidebar-accent text-sidebar-accent-foreground font-semibold shadow-md border-l-2 border-sidebar-primary" 
