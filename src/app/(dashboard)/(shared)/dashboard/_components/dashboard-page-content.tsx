...
const DashboardGrid = dynamic(
  () =>
    import("@/components/dashboard-grid").then((m) => ({
...
const AdminDashboardView = dynamic(
  () =>
    import("../admin-dashboard-view").then((m) => ({
...
const ManagerDashboardView = dynamic(
  () =>
    import("../manager-dashboard-view").then((m) => ({
      default: m.ManagerDashboardView,
    })),
  {
    ssr: false,
    loading: () => <AdminDashboardSkeleton />,
...
export function DashboardPageContent() {
  ...
  const [depositsStatistics, setDepositsStatistics] = useState<
    Array<{ day: string; date: string; amount: number }>
  >([]);
  const [withdrawalsStatistics, setWithdrawalsStatistics] = useState<
    Array<{ day: string; date: string; amount: number }>
  >([]);
  const [isDashboardLoading, setIsDashboardLoading] = useState(
    isUser || isAdmin,
  );
  const [isDepositsStatisticsLoading, setIsDepositsStatisticsLoading] =
    useState(false);
  const [isWithdrawalsStatisticsLoading, setIsWithdrawalsStatisticsLoading] =
    useState(false);
  const [dashboardError, setDashboardError] = useState<unknown | null>(null);
  const [tradingSummaryError, setTradingSummaryError] = useState<
    unknown | null
  >(null);
  const [depositStatisticsPeriod, setDepositStatisticsPeriod] = useState<
    7 | 30
  >(30);
  ...
  const forceGradientTheme = walletCardWidth > 0 && walletCardWidth <= 261;
  ...
  const dashboardNewsItems = useMemo(() => {
    ...
    return (dashboardData.latest_news as unknown as NewsItem[]).filter(
      (i) => i.type === "news",
    );
  ...
  const [resetPasswordAccount, setResetPasswordAccount] =
    useState<DashboardTradingAccount | null>(null);
  const [depositAccount, setDepositAccount] =
    useState<DashboardTradingAccount | null>(null);
  const [selectedLivePositionsLogin, setSelectedLivePositionsLogin] =
    useState("");
  ...
  useEffect(() => {
    ...
    const unseenNews = recentNews.filter(
      (item) => !seenNewsIds.has(String(item.id)),
    );
    const unseenPromos = recentPromotions.filter(
      (item) => !seenPromoIds.has(String(item.id)),
    );
  ...
  useEffect(() => {
    ...
    const fetchDepositStatistics = async () => {
      ...
      try {
        const depositsStatsResponse = await authApi.getWalletStatistics(
          token,
          "deposits",
          depositStatisticsPeriod,
        );
  ...
  useEffect(() => {
    ...
    const fetchWithdrawalStatistics = async () => {
      ...
      try {
        const withdrawalsStatsResponse = await authApi.getWalletStatistics(
          token,
          "withdrawals",
          withdrawalStatisticsPeriod,
        );
    ...
    return Object.entries(checklist).map(([key, value]) => {
      ...
      return {
        ...
        description: completed
          ...
          : "Action required to finish this section.",
        icon: completed ? (
  ...
  const mtAccountSummary: TradingAccountSummaryItem[] =
  ...
  const {
    ...
    token,
    enabled:
      isUser && !isDashboardLoading && !dashboardError && !hasAnyMt5Accounts,
  });
  ...
  const { data: accountServersById = {} } = useQuery<
    Record<number, string | null>
  >({
    queryKey: [
      "dashboardAccountServers",
      token,
      currentAccounts.map((account) => account.id).join(","),
    ],
    queryFn: async () => {
      const serverEntries = await Promise.all(
        currentAccounts.map(async (account) => {
          ...
          return [
            account.id,
            response.data?.server ?? response.data?.mt5_account?.server ?? null,
          ] as const;
        }),
      );
  ...
  const selectedLivePositionsAccount =
    currentAccounts.find(
      (account) => account.mt5_id === selectedLivePositionsLogin,
    ) ?? null;
  ...
  useEffect(() => {
    const firstAvailableLogin =
      currentAccounts.find((account) => account.mt5_id)?.mt5_id ?? "";
  ...
  const {
    ...
    queryFn: async () => {
      const response = await mt5SdkApi.getPositions(
        selectedLivePositionsLogin,
        token,
      );
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-2xl font-bold text-foreground tabular-nums">
                      {overallAccounts?.total_accounts ??
                        dashboardData?.account_types?.total_accounts ??
                        0}
                    </p>
                  </div>
                  <div className="p-3 rounded-2xl bg-muted/70 border border-border/50">
        ...
        component: isDepositsStatisticsLoading ? (
          ...
          <DashboardTrendChart
            ...
            onPeriodChange={(period) =>
              setDepositStatisticsPeriod(period === "7d" ? 7 : 30)
            }
          />
        ),
      ...
      // Withdrawals Chart Widget
      {
        ...
        component: isWithdrawalsStatisticsLoading ? (
          ...
          <DashboardTrendChart
            ...
            onPeriodChange={(period) =>
              setWithdrawalStatisticsPeriod(period === "7d" ? 7 : 30)
            }
          />
        ),
              ...
              {canCustomizeDashboard && (
                <div className="flex items-center justify-between gap-4">
                  ...
                  <div className="flex items-center gap-2 rounded-lg border border-border/50 bg-muted/50 p-1">
                    ...
                    <Button
                      variant={
                        isCustomDashboard === "custom" ? "default" : "ghost"
                      }
                      size="sm"
                  ...
                  <div className="grid grid-cols-1 xl:grid-cols-4 gap-6">
                    ...
                    >
                      ...
                      <CardHeader className="relative z-10 pb-3 px-6 pt-6">
                        <div className="flex items-center justify-between mb-2">
                          <p
                            className={`uppercase tracking-wider text-xs font-bold flex items-center gap-2 ${effectiveTheme ? "text-muted-foreground" : "text-primary-foreground/80"}`}
                          >
                            <Wallet className="h-3.5 w-3.5" />
                            Wallet Balance
                          </p>
                          <div
                            className={`p-1.5 rounded-lg backdrop-blur-sm border ${effectiveTheme ? "bg-muted/70 border-border/50" : "bg-primary-foreground/10 border-primary-foreground/20"}`}
                          >
                            <Shield
                              className={`h-4 w-4 ${effectiveTheme ? "text-foreground" : "text-primary-foreground"}`}
                            />
                          </div>
                        </div>
                        <div className="flex items-baseline gap-2 mt-2">
                          <span
                            className={`text-4xl font-extrabold leading-tight ${effectiveTheme ? "" : "drop-shadow-lg"}`}
                          >
                            {formatAmount(dashboardData?.wallet?.balance)}
                          </span>
                          <span
                            className={`text-lg font-bold ${effectiveTheme ? "text-muted-foreground" : "text-primary-foreground/80"}`}
                          >
                            USD
                          </span>
                        </div>
                      ...
                      <CardContent className="relative z-10 pt-4 pb-6 px-6">
                        <div className="flex items-start justify-between gap-4">
                          <div className="flex-1 space-y-3">
                            ...
                            <div
                              className={`flex items-center gap-2 text-xs px-3 py-1.5 rounded-lg backdrop-blur-sm border ${effectiveTheme ? "text-muted-foreground bg-muted/70 border-border/50" : "text-primary-foreground/90 bg-primary-foreground/10 border-primary-foreground/20"}`}
                            >
                          ...
                          <div className="flex flex-col items-center gap-2 flex-shrink-0">
                            <div
                              className={`p-4 rounded-2xl backdrop-blur-md shadow-lg group-hover:scale-110 transition-transform duration-300 ${effectiveTheme ? "border border-border/50 bg-muted/70" : "border-2 border-primary-foreground/30 bg-primary-foreground/10"}`}
                            >
                              <Lock
                                className={`h-8 w-8 ${effectiveTheme ? "text-foreground" : "text-primary-foreground"}`}
                              />
                            </div>
                            <span
                              className={`text-[10px] font-semibold uppercase tracking-wider ${effectiveTheme ? "text-muted-foreground" : "text-primary-foreground/90"}`}
                            >
                              Protected
                            </span>
                          </div>
              ...
              <div className="grid gap-4 xl:grid-cols-2">
                {isDepositsStatisticsLoading ? (
                      ...
                      <DashboardTrendChart
                        ...
                        emptyStateLabel="No deposit data available"
                        formatValue={(value) =>
                          formatCurrency(value, depositsCurrency)
                        }
                        selectedPeriod={
                          depositStatisticsPeriod === 7 ? "7d" : "1m"
                        }
                        onPeriodChange={(period) =>
                          setDepositStatisticsPeriod(period === "7d" ? 7 : 30)
                        }
                ...
                {isWithdrawalsStatisticsLoading ? (
                      ...
                      <DashboardTrendChart
                        ...
                        emptyStateLabel="No withdrawal data available"
                        formatValue={(value) =>
                          formatCurrency(value, withdrawalsCurrency)
                        }
                        selectedPeriod={
                          withdrawalStatisticsPeriod === 7 ? "7d" : "1m"
                        }
                        onPeriodChange={(period) =>
                          setWithdrawalStatisticsPeriod(
                            period === "7d" ? 7 : 30,
                          )
                        }
                      />
                  ...
                  headerActions={
                    <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
                      ...
                      <Button
                        ...
                        disabled={
                          !selectedLivePositionsLogin ||
                          isRefreshingLivePositions
                        }
                      >
                        <RefreshCw
                          className={`mr-2 h-4 w-4${isRefreshingLivePositions ? " animate-spin" : ""}`}
                        />
                        Refresh
                      </Button>
                    </div>
              ...
              <div className="space-y-6">
                {tradingSummaryError ? (
                  <ApiErrorState
                    ...
                    onRetry={() => {
                      void authApi
                        .getTradingAccountsSummary(token!)
                        .then((res) => {
                  ...
                  <>
                    ...
                    <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3">
                      {currentAccounts.map((account) => {
                        const isDemo = account.account_mode
                          ?.toLowerCase()
                          .includes("demo");
                        const menuActions = [
                          {
                        ...
                        return (
                          <ClientMt5AccountCard
                            ...
                            onDeposit={
                              isDemo
                                ? () => openDashboardDepositDialog(account)
                                : undefined
                            }
                  ...
                  <>
                    <div className="space-y-2">
                      <h2 className="text-2xl font-bold text-foreground">
                        Open Your First MT5 Account
                      </h2>
                      <p className="max-w-2xl text-muted-foreground">
                        You don&apos;t have any MT5 accounts yet. Choose an
                        account type below and create a live or demo login
        ...
        <ProfileCompletionDialog
          ...
          onOpenChange={(open) => {
            ...
            if (open === false) {
              ...
              const hasSeenMt5Dialog =
                sessionStorage.getItem("mt5_dialog_shown");
