...
export default function WalletOverviewPage() {
  ...
  const [brokerCryptoWallet, setBrokerCryptoWallet] =
    useState<BrokerCryptoWalletItem | null>(null);
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        if (response.data.mt5_wallets && response.data.mt5_wallets.length > 0) {
          const balancePromises = response.data.mt5_wallets.map(
            async (wallet) => {
              try {
                ...
                console.error(
                  `[Wallet Overview] ❌ Error fetching balance for MT5 ${wallet.mt5_id}:`,
                  error,
                );
                return { mt5Id: wallet.mt5_id, balance: null };
              }
  ...
  const getTransactionCurrency = (transaction: {
    mt5_user_id?: string | number | null;
  }) => {
  ...
  const getTransactionDisplayAmount = (transaction: {
  ...
  const getTransactionCurrencySymbol = (transaction: {
    mt5_user_id?: string | number | null;
  }) => {
    const currency = getTransactionCurrency(transaction);
    return currency === "USC" ? "¢" : "$";
            ...
            <CardHeader className="relative z-10 pb-0 px-0">
              ...
              <div className="flex items-baseline gap-2 mt-2">
                <span className="text-4xl font-black leading-tight">
                  {walletData
                    ? formatAmount(mainWallet?.balance?.toFixed(2) ?? "0.00")
                    : "-"}
                </span>
              ...
              <div className="flex items-start justify-between gap-6">
                <div className="flex-1">
                  <p className="text-primary-foreground/75 text-sm leading-relaxed">
                    Securely store and manage balances across deposits,
                    withdrawals, and transfers in one place.
                  </p>
                ...
                {walletData.recent_transactions.map((transaction) => (
                  ...
                  >
                    <div className="flex items-center gap-4 flex-1 min-w-0">
                      <div
                        className={`p-3 rounded-xl transition-all duration-300 flex-shrink-0 ${
                          ...
                          transaction.type.includes("ib_commission")
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1.5 min-w-0">
                          ...
                          <Badge
                            variant="outline"
                            className="text-xs border-primary/20 bg-primary/5 shrink-0"
                          >
                            {transaction.wallet_type === "ib"
                              ? "partner"
                    ...
                    <div className="text-right ml-4">
                      ...
                      >
                        {isCreditTransaction(transaction.type) ? "+" : "-"}
                        {getTransactionCurrencySymbol(transaction)}
