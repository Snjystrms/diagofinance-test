...
import {
  userReportApi,
  type UserIbWithdrawalReportItem,
  type UserIbWithdrawalReportPayload,
} from "@/lib/api";
...
const statusBadge = (status: string) => {
  ...
  if (v === "approved") return <Badge className="bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300 border-green-300 dark:border-green-800">Approved</Badge>;
...
const destinationBadge = (destination: string) => {
  ...
  if (v === "bank") return <Badge className="bg-sky-100 text-sky-800 dark:bg-sky-900/30 dark:text-sky-300 border-sky-300 dark:border-sky-800">Bank</Badge>;
  if (v === "mt5") return <Badge className="bg-violet-100 text-violet-800 dark:bg-violet-900/30 dark:text-violet-300 border-violet-300 dark:border-violet-800">MT5</Badge>;
  return <Badge variant="outline">{destination || "—"}</Badge>;
};

export default function UserIbWithdrawReportPage() {
  const authCtx = useAuth?.();
  const ctxToken = authCtx?.token;
  const token = ctxToken || (typeof window !== "undefined" ? localStorage.getItem("auth_token") || "" : "");

  const [rows, setRows] = useState<UserIbWithdrawalReportItem[]>([]);
  ...
  const loadReport = useCallback(async () => {
    ...
    try {
      setLoading(true);
      setLoadError(null);
      const response = await userReportApi.ibWithdrawal.list({
        token,
        page,
        per_page: perPage,
      ...
      const payload = response as unknown as UserIbWithdrawalReportPayload;
      ...
      if (requestId !== requestIdRef.current) return;
      console.error("Failed to load user IB withdrawal report:", error);
      setLoadError(error);
      toast.error(getFriendlyErrorMessage(error, { resource: "IB withdrawal report", action: "load" }));
      setRows([]);
    } finally {
      if (requestId === requestIdRef.current) setLoading(false);
    }
  }, [token, page, perPage, destinationFilter, statusFilter, fromDate, toDate, sortBy, sortOrder, searchQuery]);
  ...
  const handleResetFilters = useCallback(() => {
    ...
    setSearchQuery(null);
    setPage(1);
  }, [setDestinationFilter, setStatusFilter, setSortBy, setSortOrder, setSearchQuery, setPage]);
  ...
  const handleExport = useCallback(
    async (formatType: "xlsx" | "csv") => {
      ...
      try {
        ...
        const { blob, filename } = await userReportApi.ibWithdrawal.export({
          token,
          format: formatType,
        ...
        toast.error(getFriendlyErrorMessage(error, { resource: "IB withdrawal report", action: "export" }), { id: exportToastId });
      }
    },
    [token, destinationFilter, statusFilter, fromDate, toDate, searchQuery],
  );
  ...
  const activeFilterCount = useMemo(() => {
    ...
    return count;
  }, [destinationFilter, statusFilter, fromDate, toDate, sortBy, sortOrder, searchQuery]);

  const columns: ColumnDef<UserIbWithdrawalReportItem>[] = useMemo(
    () => [
      {
        id: "id",
      ...
      },
      {
        ...
        cell: ({ row }) => {
          ...
          return <span className="font-mono text-xs max-w-[200px] truncate block" title={String(v)}>{String(v)}</span>;
        },
      },
      {
        id: "amount",
  ...
  if (loadError && rows.length === 0) {
    return (
      <div className="container mx-auto px-4 py-10 md:px-6 lg:px-8">
        <ApiErrorState error={loadError} audience="client" variant="panel" resource="IB withdrawal report" action="load" onRetry={() => void loadReport()} />
      </div>
    );
  }
    ...
    <div className="container mx-auto p-6 space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-0.5">Your IB commission withdrawals to bank or MT5</p>
        </div>
      ...
      <div className="rounded-lg border bg-card">
        <div className="p-5">
          ...
          <AppDataTable<UserIbWithdrawalReportItem> data={rows} columns={columns} pageCount={totalPages} />
        </div>
      </div>
    </div>
  );
