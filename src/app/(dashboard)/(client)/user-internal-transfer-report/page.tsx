...
import {
  userReportApi,
  type UserInternalTransferReportItem,
  type UserInternalTransferReportPayload,
} from "@/lib/api";
...
const statusBadge = (status: string) => {
  ...
  if (v === "completed" || v === "success") return <Badge className="bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300 border-green-300 dark:border-green-800">Completed</Badge>;
  if (v === "pending") return <Badge variant="secondary" className="bg-yellow-100 text-yellow-800 dark:bg-yellow-900/30 dark:text-yellow-400 border-yellow-300 dark:border-yellow-800">Pending</Badge>;
  if (v === "rejected" || v === "failed") return <Badge variant="destructive">Failed</Badge>;
  return <Badge variant="outline">{status || "—"}</Badge>;
};

const accountBadge = (account: string | null | undefined) => {
  if (!account) return <span className="text-muted-foreground">—</span>;
  const v = String(account).toLowerCase();
  if (v.includes("mt5")) return <Badge className="bg-violet-100 text-violet-800 dark:bg-violet-900/30 dark:text-violet-300 border-violet-300 dark:border-violet-800">MT5</Badge>;
  if (v.includes("wallet")) return <Badge className="bg-sky-100 text-sky-800 dark:bg-sky-900/30 dark:text-sky-300 border-sky-300 dark:border-sky-800">Wallet</Badge>;
  return <Badge variant="outline">{account}</Badge>;
};

export default function UserInternalTransferReportPage() {
  const authCtx = useAuth?.();
  const ctxToken = authCtx?.token;
  const token = ctxToken || (typeof window !== "undefined" ? localStorage.getItem("auth_token") || "" : "");

  const [rows, setRows] = useState<UserInternalTransferReportItem[]>([]);
  ...
  const loadReport = useCallback(async () => {
    ...
    try {
      setLoading(true);
      setLoadError(null);
      const response = await userReportApi.internalTransfer.list({
        token,
        page,
        per_page: perPage,
      ...
      const payload = response as unknown as UserInternalTransferReportPayload;
      ...
      if (requestId !== requestIdRef.current) return;
      console.error("Failed to load user internal transfer report:", error);
      setLoadError(error);
      toast.error(getFriendlyErrorMessage(error, { resource: "internal transfer report", action: "load" }));
      setRows([]);
    } finally {
      if (requestId === requestIdRef.current) setLoading(false);
    }
  }, [token, page, perPage, statusFilter, fromDate, toDate, sortBy, sortOrder, searchQuery]);
  ...
  const handleResetFilters = useCallback(() => {
    ...
    setSearchQuery(null);
    setPage(1);
  }, [setStatusFilter, setSortBy, setSortOrder, setSearchQuery, setPage]);
  ...
  const handleExport = useCallback(
    async (formatType: "xlsx" | "csv") => {
      ...
      try {
        ...
        const { blob, filename } = await userReportApi.internalTransfer.export({
          token,
          format: formatType,
          status: statusFilter || undefined,
          from_date: fromDate ? format(fromDate, "yyyy-MM-dd") : undefined,
        ...
        toast.error(getFriendlyErrorMessage(error, { resource: "internal transfer report", action: "export" }), { id: exportToastId });
      }
    },
    [token, statusFilter, fromDate, toDate, searchQuery],
  );
  ...
  const activeFilterCount = useMemo(() => {
    ...
    return count;
  }, [statusFilter, fromDate, toDate, sortBy, sortOrder, searchQuery]);

  const columns: ColumnDef<UserInternalTransferReportItem>[] = useMemo(
    () => [
      {
        id: "id",
      ...
      },
      {
        ...
        cell: ({ row }) => accountBadge(row.original.from_account),
      },
      {
        id: "to_account",
        header: "To",
        accessorKey: "to_account",
        cell: ({ row }) => accountBadge(row.original.to_account),
      },
      {
        id: "amount",
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <div className="flex items-center gap-1 text-sm whitespace-nowrap">
            ...
            <span>{fmtDateTime(row.original.date || row.original.created_at)}</span>
          </div>
        ),
      },
    ],
  ...
  if (loadError && rows.length === 0) {
    return (
      <div className="container mx-auto px-4 py-10 md:px-6 lg:px-8">
        <ApiErrorState error={loadError} audience="client" variant="panel" resource="internal transfer report" action="load" onRetry={() => void loadReport()} />
      </div>
    );
  }
    ...
    <div className="container mx-auto p-6 space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-0.5">Your internal transfer history</p>
        </div>
      ...
      <div className="flex flex-col gap-4 md:flex-row md:flex-wrap md:items-end">
        <div className="min-w-[240px] flex-1">
          <ApiSearchBar
            ...
            placeholder="Search by amount or comment..."
            minimumLength={2}
            delay={300}
          />
        </div>
      ...
      <div className="rounded-lg border bg-card">
        <div className="p-5">
          ...
          <AppDataTable<UserInternalTransferReportItem> data={rows} columns={columns} pageCount={totalPages} />
        </div>
      </div>
    </div>
  );
