...
import {
  userReportApi,
  type UserWithdrawalReportItem,
  type UserWithdrawalReportPayload,
} from "@/lib/api";
...
export default function UserWithdrawalReportPage() {
  ...
  const loadReport = useCallback(async () => {
    ...
    try {
      setLoading(true);
      setLoadError(null);
      const response = await userReportApi.withdrawal.list({
        token,
        page,
        per_page: perPage,
      ...
      const payload = response as unknown as UserWithdrawalReportPayload;
      ...
      if (requestId !== requestIdRef.current) return;
      console.error("Failed to load user withdrawal report:", error);
      setLoadError(error);
      toast.error(getFriendlyErrorMessage(error, { resource: "withdrawal report", action: "load" }));
      setRows([]);
    } finally {
  ...
  const handleExport = useCallback(
    async (formatType: "xlsx" | "csv") => {
      ...
      try {
        ...
        const { blob, filename } = await userReportApi.withdrawal.export({
          token,
          format: formatType,
          status: statusFilter || undefined,
          from_date: fromDate ? format(fromDate, "yyyy-MM-dd") : undefined,
        ...
        toast.error(getFriendlyErrorMessage(error, { resource: "withdrawal report", action: "export" }), { id: exportToastId });
      }
    },
  ...
  const columns: ColumnDef<UserWithdrawalReportItem>[] = useMemo(
    () => [
      {
        id: "id",
      ...
      },
      {
        ...
        accessorKey: "amount",
        cell: ({ row }) => <span className="font-medium whitespace-nowrap text-red-600">{formatAmount(row.original.amount)}</span>,
      },
      {
        id: "withdraw_to",
        header: "Withdraw To",
      ...
      },
      {
        ...
        cell: ({ row }) => row.original.chain_id || "—",
      },
      {
        id: "transaction_hash",
        header: "Tx Hash",
        ...
        cell: ({ row }) => {
          const v = row.original.transaction_hash;
          if (!v) return <span className="text-muted-foreground">—</span>;
  ...
  if (loadError && rows.length === 0) {
    return (
      <div className="container mx-auto px-4 py-10 md:px-6 lg:px-8">
        <ApiErrorState error={loadError} audience="client" variant="panel" resource="withdrawal report" action="load" onRetry={() => void loadReport()} />
      </div>
    );
  }
    ...
    <div className="container mx-auto p-6 space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-0.5">Your withdrawal request history</p>
        </div>
      ...
      <div className="flex flex-col gap-4 md:flex-row md:flex-wrap md:items-end">
        <div className="min-w-[240px] flex-1">
          <ApiSearchBar
            ...
            placeholder="Search by amount, hash or address..."
            minimumLength={2}
            delay={300}
          />
        </div>
      ...
      <div className="rounded-lg border bg-card">
        <div className="p-5">
          ...
          <AppDataTable<UserWithdrawalReportItem> data={rows} columns={columns} pageCount={totalPages} />
        </div>
      </div>
    </div>
  );
