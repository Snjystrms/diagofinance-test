...
const CLIENT_USERS_BY_LEVEL_ENDPOINT = "/user/ib/users-by-level";
...
export const fetchClientUsersByLevel = async (
  token: string,
  options?: {
    page?: number;
    page_size?: number;
  ...
  const url = `${API_BASE_URL}${CLIENT_USERS_BY_LEVEL_ENDPOINT}?${params.toString()}`;
  try {
    const res = await fetch(url, {
      headers: { Authorization: `Bearer ${token}` },
      cache: "no-store",
    ...
    if (!res.ok || !json?.success || !json?.data) {
      ...
      if (options?.throwOnError) {
        throw new ApiRequestError({
          ...
          status: res.status,
          statusText: res.statusText,
          endpoint: CLIENT_USERS_BY_LEVEL_ENDPOINT,
          payload: json ?? null,
        });
      }
      toast.error(
    ...
    console.error("Failed to fetch client users by level:", error);
    toast.error(
      getFriendlyErrorMessage(error, { resource: "downline users", action: "load" })
    );
    return null;
  }
};
